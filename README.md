...
### ▶ [**Live demo**](https://cycle-website-pi.vercel.app/) · [API docs](https://cyclehaven-api.onrender.com/swagger-ui.html) · [API health](https://cyclehaven-api.onrender.com/actuator/health)

Browse the catalogue, build a cart and check out without creating an account. Card
validation is real; no payment provider is involved, so use `4242 4242 4242 4242` with any
future expiry and any CVV. A number ending `0002` simulates a decline.
...
Three free services, none of which expire:

| Piece | Runs on | Notes |
|---|---|---|
| **Frontend** | Vercel | Static build on a CDN, no cold start |
| **API** | Render | Docker image built from `backend/Dockerfile` |
| **Database** | Neon | Serverless PostgreSQL |

The database deliberately isn't on Render: its free PostgreSQL expires 30 days after
creation, which would break a demo link a month after sharing it without any warning.
Neon's free plan has no expiry.

Both halves are described in version control — [`render.yaml`](./render.yaml) defines the
API service and every environment variable it needs, [`frontend/vercel.json`](./frontend/vercel.json)
handles SPA routing and asset caching — so the deployment is reproducible rather than a
sequence of remembered dashboard clicks. Secrets are never in the repo: `JWT_SECRET` is
generated by Render, and the database credentials are entered once at provisioning time.
Full walkthrough in [DEPLOYMENT.md](./DEPLOYMENT.md).

Render's free tier sleeps a service after 15 minutes without traffic, so an uptime monitor
pings `/actuator/health` every 5 minutes to keep it warm. That endpoint also probes the
database, so a green check means the whole chain is alive rather than just the web server.

### One bug worth naming

The admin sales report worked against H2 in every test and returned 500 in production. The
query folded its optional filter into SQL:

```sql
WHERE :email IS NULL OR LOWER(COALESCE(u.email, o.guestEmail)) LIKE CONCAT('%', :email, '%')
```

With no filter, `:email` binds as an untyped null. Its only appearance is inside `IS NULL`,
which accepts any type, so PostgreSQL cannot infer one and refuses the statement. H2 infers
a type and runs it. Requesting the same endpoint *with* a filter returned 200, which
isolated the cause to the null branch. The fix was to stop asking SQL to decide whether a
filter applies — that choice now happens in Java, and each case runs its own query.
